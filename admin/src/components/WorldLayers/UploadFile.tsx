...
class UploadFile extends React.Component {
    ...
    updateFilesList = (reqFiles: IReqFile[]) => {
        ...
        reqFiles.map((reqFile: IReqFile) => {
            ...
            if (extension.includes('tif') || extension === '.shp' || extension === '.jpg' || extension === '.jpeg'){
                ...
                if (layerIndex !== -1){
                    ...
                    if (reqFile.fileType === 'image'){
                        this.uploadFiles[layerIndex].tags = reqFile.tags;
                    }
    ...
    getImageData = () => {
        ...
        const imageList: IWorldLayer[] =
            this.uploadFiles.map( (image: IFileData): IWorldLayer => {
            // get the geoData for Ansyn
            const centerPoint = [image.tags.GPSLongitude, image.tags.GPSLatitude];
            const bbox = this.getBboxFromPoint(centerPoint);
            const footprint = turf.bboxPolygon(bbox);
            // get the JPG dates as strings
            image.tags.JPGModifiedDate = new Date(image.tags.ModifyDate).toISOString();
            image.tags.JPGOriginalDate = new Date(image.tags.DateTimeOriginal).toISOString();
                
            return {
                workspaceName: this.props.world.workspaceName,
                name: this.getFileName(image.name),
                fileName: image.name,
        ...
        console.log('getImageData imageData: ' + JSON.stringify(imageList));
        this.saveLayersToDataBase(imageList);
    };

    getBboxFromPoint = (centerPoint: number[]): BBox => {
        const polygonSize = 400;                            // in meters
        const distance: number = polygonSize/1000;          // convert to kilometers
